...
int InputInt()
{
	...
	if (cin.fail())
	{
		cin.clear();
		cin.ignore(1024, '\n');
		return INT_MAX; // INT_MAX가 리턴되면 입력이 잘못되었단 뜻
	}
	return iInput;
}

// 메인 메뉴를 화면에 보여주고 입력받은 메뉴를 반환한다.
// 입력 오류일 경우 MM_NONE을 반환해주고
int OutputMainMenu() {
	system("cls");
	cout << "*************************** 로비 ***************************" << endl;
	cout << "1. 맵" << endl;
...
int main() {
	...
	g_tLvUpTable[JOB_KNIGHT - 1] = CreateLvUPStatus(40, 10,8, 16, 50, 100, 10, 20);
